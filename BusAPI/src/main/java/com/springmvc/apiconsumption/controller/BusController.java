...
import com.springmvc.apiconsumption.model.Stop;
import com.springmvc.apiconsumption.model.Time;
...
public class BusController {
	...
	public ModelAndView submitBusForm(@ModelAttribute("busObj") Bus busObj) {
		...
		BusRoute[] busRouteObj = restTemplate.getForObject("http://svc.metrotransit.org/NexTrip/Routes", BusRoute[].class);
		...
		/**
		 * Pass the RouteID and the direction from the above 2
		 * to GetStops to get Stop value 
		 */
		...
		/**
		 * Send RouteID, StopName and Direction to
		 * GetTimePointDepartures
		 * Return DepartuteText and DepartureTime
		 */
